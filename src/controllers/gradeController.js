...
function buildUpdateData(body, currentEnrollment, distribution = null) {
    ...
    if (distribution) {
        ...
        const effectiveMid =
            updateData.mid_score ?? currentEnrollment.mid_score;
        const effectiveWork =
            updateData.work_score ?? currentEnrollment.work_score;
...
export const getMyCgpaTrend = async (req, res) => {
    try {
        ...
        const sorted = Array.from(semesterMap.values()).sort((a, b) => {
            ...
            return (
                (SEMESTER_ORDER[a.semester] ?? 0) -
                (SEMESTER_ORDER[b.semester] ?? 0)
            );
        });
