  ...
  try {
    ...
    const formattedTeachers = teachers.map((teacher) => {
      ...
      if (
        ...
        department =
          teacher.tutorials_labs[0].course_offerings.courses.departments.name;
      }
...
export const getTeacherSchedule = async (req, res) => {
  try {
    ...
    if (user.userId !== teacherId && user.id !== teacherId) {
      return res
        .status(403)
        .json({ error: "Access denied. You can only view your own schedule." });
    }
    ...
    // Add office hours
    teacher.office_hours.forEach((officeHour) => {
      const day = normalizeDayName(officeHour.day_of_week);
      if (!scheduleMap.has(day)) {
        scheduleMap.set(day, []);
      }
      scheduleMap.get(day).push({
        ...
        courseCode: null,
        courseName: null,
        location: officeHour.location || "TBA",
        type: "office_hours",
      });
    });
