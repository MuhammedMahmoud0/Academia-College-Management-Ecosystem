...
export default function AddMaterialModal({ isOpen, onClose, onSave }) {
  ...
  const handleSubmit = () => {
    if (uploadType === 'file' && file) {
      onSave({ section, type: 'file', file });
    } else if (uploadType === 'link' && link) {
      onSave({ section, type: 'link', link });
    }
    handleClose();
  };

  const handleClose = () => {
    setSection('Lecture Materials');
    setUploadType('file');
    setFile(null);
    setLink('');
    setIsDragging(false);
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md sm:max-w-lg md:max-w-xl overflow-hidden">
        ...
        <div className="flex justify-end gap-3 p-4 sm:p-5 md:p-6 border-t border-gray-200 bg-gray-50">
          ...
          <button
            ...
            disabled={uploadType === 'file' ? !file : !link}
            className="px-4 sm:px-5 py-2 sm:py-2.5 text-sm sm:text-base font-medium text-white bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-400 disabled:cursor-not-allowed rounded-lg transition-colors"
          >
            Save Material
          </button>
        </div>
